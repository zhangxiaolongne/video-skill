...
class TimelineStructuralRole(str, Enum):
    hook = "hook"
    build = "build"
    payoff = "payoff"
    bridge = "bridge"
...
class TimelineDurationVariant(BaseModel):
    ...
    role_allocation: dict[str, float] = Field(default_factory=dict)
    rationale: list[str] = Field(default_factory=list)
    risks: list[str] = Field(default_factory=list)


class TimelineDroppedClip(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class TimelineContinuityCheck(BaseModel):
    model_config = ConfigDict(extra="forbid")

    from_segment_id: str
    to_segment_id: str
    status: str = Field(
        pattern=r"^(same_source_continuous|same_source_jump|cross_source_cut|audio_only_transition)$"
    )
...
class TimelineSegment(BaseModel):
    ...
    structural_role: TimelineStructuralRole = TimelineStructuralRole.build
...
class TimelineDraft(BaseModel):
    ...
    selected_duration_option_id: str = ""
    structure_strategy: list[str] = Field(default_factory=list)
    duration_variants: list[TimelineDurationVariant] = Field(default_factory=list)
    segments: list[TimelineSegment] = Field(min_length=1)
    dropped_clips: list[TimelineDroppedClip] = Field(default_factory=list)
    continuity_checks: list[TimelineContinuityCheck] = Field(default_factory=list)
    music_plan: TimelineMusicPlan
    evidence: list[EvidenceRef] = Field(min_length=1)
    risks: list[str] = Field(default_factory=list)
    warnings: list[str] = Field(default_factory=list)
    no_fabricated_content_claims: bool = True
...
class TimelineValidationReport(BaseModel):
    ...
    structural_roles_present: list[str] = Field(default_factory=list)
    dropped_clip_count: int = Field(ge=0, default=0)
    continuity_check_count: int = Field(ge=0, default=0)
