...
def doctor_project_payload(project_path: Path) -> dict:
    ...
    provider_registry = proposal_provider_registry_summary(
        root / WORKSPACE_DIR / DATA_DIR / "proposal_provider_registry.json"
    )
    mock_handshake = proposal_mock_adapter_handshake_summary(
        root / WORKSPACE_DIR / DATA_DIR / "proposal_mock_adapter_handshake.json"
    )
    approval_request = proposal_execution_approval_request_summary(
        root / WORKSPACE_DIR / DATA_DIR / "proposal_execution_approval_request.json"
    )
    ...
    if (
        sources.get("valid") is True
        and provider_registry.get("valid") is False
    ):
        registry_path = root / WORKSPACE_DIR / DATA_DIR / "proposal_provider_registry.json"
        issues.append(
            workspace_issue(
                code="proposal_provider_registry_invalid",
                severity="error",
                detail=str(
                    provider_registry.get("error")
                    or "proposal provider registry is invalid"
                ),
                next_action=(
                    f"fix {registry_path.relative_to(root).as_posix()} or rerun "
                    f"artist-portrait propose --project {project_path}"
                ),
            )
        )
    if (
        sources.get("valid") is True
        and mock_handshake.get("valid") is False
    ):
        handshake_path = root / WORKSPACE_DIR / DATA_DIR / "proposal_mock_adapter_handshake.json"
        issues.append(
            workspace_issue(
                code="proposal_mock_adapter_handshake_invalid",
                severity="error",
                detail=str(
                    mock_handshake.get("error")
                    or "proposal mock adapter handshake is invalid"
                ),
                next_action=(
    ...
    if (
        sources.get("valid") is True
        and approval_request.get("valid") is False
    ):
        approval_path = (
            root / WORKSPACE_DIR / DATA_DIR / "proposal_execution_approval_request.json"
        )
        issues.append(
            workspace_issue(
                code="proposal_execution_approval_request_invalid",
                severity="error",
                detail=str(
                    approval_request.get("error")
                    or "proposal execution approval request is invalid"
                ),
                next_action=(
                    f"fix {approval_path.relative_to(root).as_posix()} or rerun "
                    f"artist-portrait propose --project {project_path}"
                ),
            )
        )
    if (
        sources.get("valid") is True
        and approval_record.get("valid") is False
    ):
        ...
        issues.append(
            workspace_issue(
                code="proposal_execution_approval_record_invalid",
                severity="error",
                detail=str(
                    approval_record.get("error")
                    or "proposal execution approval record is invalid"
                ),
                next_action=(
...
def artifact_statuses(root: Path) -> dict[str, dict]:
    artifact_paths = {
        ...
        "proposal_provider_registry": root
        / WORKSPACE_DIR
        / DATA_DIR
        / "proposal_provider_registry.json",
        "proposal_mock_adapter_handshake": root
        / WORKSPACE_DIR
        / DATA_DIR
        / "proposal_mock_adapter_handshake.json",
        "proposal_execution_approval_request": root
        / WORKSPACE_DIR
        / DATA_DIR
        / "proposal_execution_approval_request.json",
        "proposal_execution_approval_record": root
        / WORKSPACE_DIR
        / DATA_DIR
...
def status_summaries(root: Path) -> dict:
    ...
    proposal_provider_registry_path = (
        root / WORKSPACE_DIR / DATA_DIR / "proposal_provider_registry.json"
    )
    proposal_mock_adapter_handshake_path = (
        root / WORKSPACE_DIR / DATA_DIR / "proposal_mock_adapter_handshake.json"
    )
    proposal_execution_approval_request_path = (
        root / WORKSPACE_DIR / DATA_DIR / "proposal_execution_approval_request.json"
    )
...
def read_proposal_provider_registry_json(path: Path) -> ProposalProviderRegistry:
    try:
        return ProposalProviderRegistry.model_validate_json(path.read_text(encoding="utf-8"))
    except ValueError as exc:
        raise WorkspacePrerequisiteError(
            f"invalid ProposalProviderRegistry JSON: {exc}"
        ) from exc


def read_proposal_mock_adapter_handshake_json(path: Path) -> ProposalMockAdapterHandshake:
    try:
        return ProposalMockAdapterHandshake.model_validate_json(
...
def read_proposal_execution_approval_request_json(
    ...
    try:
        return ProposalExecutionApprovalRequest.model_validate_json(
            path.read_text(encoding="utf-8")
        )
    except ValueError as exc:
        raise WorkspacePrerequisiteError(
            f"invalid ProposalExecutionApprovalRequest JSON: {exc}"
        ) from exc
...
def read_proposal_execution_approval_record_json(
    ...
    try:
        return ProposalExecutionApprovalRecord.model_validate_json(
            path.read_text(encoding="utf-8")
        )
    except ValueError as exc:
        raise WorkspacePrerequisiteError(
