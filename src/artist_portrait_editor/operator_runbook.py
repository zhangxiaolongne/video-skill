...
def build_operator_runbook(
    *,
    ...
    workflow_repair = _read_optional(
        root / WORKSPACE_DIR / DATA_DIR / "workflow_repair_plan.json",
        WorkflowRepairPlan,
    )
    workflow_refresh = _read_optional(
        root / WORKSPACE_DIR / DATA_DIR / "workflow_repair_refresh_plan.json",
        WorkflowRepairRefreshPlan,
    )
...
def _artifact_map(
    *,
    ...
    workflow_repair: WorkflowRepairPlan | None,
    workflow_refresh: WorkflowRepairRefreshPlan | None,
    ...
    specs = [
        ...
        ("workflow_repair_plan", ".artist-portrait/data/workflow_repair_plan.json", workflow_repair, workflow_repair is not None),
        ("workflow_repair_refresh", ".artist-portrait/data/workflow_repair_refresh_plan.json", workflow_refresh, workflow_refresh is not None),
        ...
        ("fcpxml_repair_dry_run", ".artist-portrait/data/fcpxml_repair_dry_run.json", None, True),
        ("fcpxml_repair_execution_review", ".artist-portrait/data/fcpxml_repair_execution_review.json", None, True),
    ]
...
def _operator_stages(
    *,
    ...
    _append_if_present(
        ...
        "workflow",
        "workflow",
        workflow_repair.status if workflow_repair else None,
        "artist-portrait workflow --project <project.yaml> --target delivery --repair-plan",
        "Workflow execution gaps have an ordered repair plan.",
        [".artist-portrait/data/workflow_repair_plan.json", "output/workflow_repair_plan.md"],
    )
    _append_if_present(
        stages,
        "workflow_repair_refresh",
        "workflow",
        "workflow",
        workflow_refresh.status if workflow_refresh else None,
        "artist-portrait workflow --project <project.yaml> --target delivery --repair-refresh-plan",
        "Accepted workflow repair evidence can be resubmitted.",
        [".artist-portrait/data/workflow_repair_refresh_plan.json", "output/workflow_repair_refresh_plan.md"],
    )
    return stages
