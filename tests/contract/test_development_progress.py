...
def test_machine_readable_progress_matches_current_dashboard():
    ...
    assert "Text, Subtitle, And On-Screen Timing Plan" in progress
    assert payload["tasks"][0]["id"] == "canonical_text_plan"
    assert payload["tasks"][-1]["id"] == "real_validation"
