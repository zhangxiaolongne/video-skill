...
def test_six_canonical_document_owners_exist_and_are_linked():
    ...
    assert (DOCS / "archive" / "DECISIONS_HISTORY.md").is_file()
    assert (DOCS / "archive" / "RELEASES_HISTORY.md").is_file()
...
def test_progress_is_dashboard_not_duplicate_history():
    ...
    assert "Full release history is archived" in releases
    ...
    assert len(releases.splitlines()) < 120
...
def test_machine_readable_progress_matches_current_dashboard():
    ...
    assert payload["archive_policy"]["release_history"] == (
        "docs/archive/RELEASES_HISTORY.md"
    )
    ...
    assert "Multi-Version Creative Strategies" in progress
    assert payload["tasks"][0]["id"] == "four_strategy_contract"
    assert payload["tasks"][-1]["id"] == "real_cross_project_validation"
