...
The current V0-010m gate uses `.artist-portrait/state.json` as a step ledger,
...
Stage A initialized ledger entries for future V0 steps. V0-010m opens only the
...
provider registry/mock handshake, no-approval execution request, no-grant approval record, no-execution readiness plan, no-call execution authorization, provider
...
`proposal_execution_readiness_plan_invalid` for malformed execution readiness plan packets,
