...
### DEC-062: Revision promotion is explicit canonical mutation, not rendering

- Recorded: `2026-07-04`
- Status: `active`
- Decision: `V1-08` introduces `artist-portrait promote-revision` as the only
  current gate allowed to promote a validated revision application into
  `output/timeline_draft.json`. The command must require an explicit
  `revision_application_id`, verify baseline timeline freshness, write one
  canonical `.artist-portrait/data/revision_promotion.json`, write one human
  `output/revision_promotion.md`, and invalidate stale downstream artifacts.
- Rationale: V1-07 deliberately stopped at a reviewable revised candidate. A
  usable revision loop needs a separate, auditable mutation step before preview
  or final export can regenerate revised media.
- Consequence: promotion may mutate the canonical timeline and move canonical
  edit points by adopting revised candidate segments. It still must not render
  media, select or fit music automatically, call models from the CLI, access
  the network, use image generation/editing, fabricate beats, or claim revised
  preview/final media exists before render commands run.
- Revisit when: a future gate adds real NLE application, automatic
  beat-synced editing, or a stronger model/vision-assisted revision engine.

### DEC-061: Revision application creates a candidate, not a promoted timeline

- Recorded: `2026-07-04`
- Status: `active`
- Decision: `V1-07` introduces `artist-portrait apply-revision` as a
  controlled revision-application gate that writes one canonical
  `.artist-portrait/data/revision_application.json` and one human
  `output/revision_application.md`. The artifact may contain revised candidate
  segments and applied/manual/conflict action results, but it must not overwrite
  `output/timeline_draft.json` or claim preview/final media has been
  regenerated.
- Rationale: V1-06 could plan revisions but could not produce a concrete
  candidate timeline. The next useful step is a reviewable candidate with strong
  provenance, not silent mutation of the canonical timeline.
- Consequence: downstream preview/export/rhythm artifacts become stale only if
  a candidate is later promoted or rendered. V1-07 may report that freshness
  risk, but it must not auto-render, auto-fit BGM, select music, call models,
  access the network, or move canonical edit points.
- Revisit when: a future gate explicitly opens revised-candidate promotion,
  revised preview rendering, or editor/NLE application of a selected candidate.
...
### DEC-058: V1 progress is measured by aesthetic editing maturity

- Recorded: `2026-07-02`
- Status: `active`
- Decision: after `v0.28.0`, the next capability program should be V1 aesthetic
  editor maturity, starting with `V1-01 Duration decision and edit brief`, not
  another isolated V0 artifact, schema, status field, review rule, or release
  packaging task
- Rationale: the real-video trial proved the pipeline can render a valid final
  MP4, but it also proved that engineering validity is not mature editing:
  fixed 10-second windows, weak content understanding, no duration
  recommendation, no transcript/vision-led shot scoring, no beat grid, and no
  second-pass aesthetic review are still major product gaps
- Consequence: future batches must map to one of the V1 aesthetic stages:
  duration decision, content understanding, aesthetic timeline generation,
  sound/BGM decision, aesthetic review/second pass, or user revision/version
  comparison. Supporting fields, schemas, tests, and bug fixes count only inside
  those stage outcomes.
- Revisit when: V1 can recommend or accept duration, generate a non-mechanical
  timeline from real content evidence, render a final cut, review its own edit,
  and respond to user revision intent with a comparable new version

### DEC-059: Edit brief is the V1 creative front door

- Recorded: `2026-07-03`
- Status: `active`
- Decision: V1 creative work must pass through a canonical edit brief before
  proposal/timeline generation. The brief owns target duration selection,
  platform assumptions, evidence level, editing intent, pacing strategy,
  BGM/source-audio boundaries, risks, and next commands.
- Rationale: the real-video trial showed that a valid MP4 is not enough; the
  system must know whether the cut is a 30-second hook, a 60-second balanced
  portrait, or a longer context cut before it asks the host Agent for proposals
  or builds timelines.
- Consequence: `artist-portrait brief` writes one canonical machine JSON and
  one Markdown report, updates state/run audit, and invalidates downstream
  creative/media outputs when the brief changes. It does not render media,
  mutate timelines, select or fit music automatically, call models from the
  CLI, access the network, or use image generation/editing.
- Revisit when: `V1-02` and `V1-03` make proposal/timeline generation consume
  the edit brief directly for clip scoring and non-mechanical timeline planning

### DEC-060: V1 JSON surface must stay converged

- Recorded: `2026-07-03`
- Status: `active`
- Decision: V1 capability gates default to one canonical machine JSON plus one
  human Markdown report. Extra JSON artifacts require a specific boundary:
  external candidate quarantine/import, generated schema contract, media
  validation, run audit, or release-critical repair/evidence review. JSON files
  are governed by four tiers: canonical truth, boundary evidence, merged
  presentation, and disposable/generated evidence.
- Rationale: V0 accumulated many JSON files because it needed explicit evidence
  chains. V1 is now building editing intelligence; copying handoff, review,
  status, or summary fields into separate JSON files makes the skill harder to
  operate without improving creative quality.
- Consequence: ordinary V1 handoff/review/status/summary data must be merged
  into the canonical artifact or Markdown. New JSON files need release-level
  justification in the batch plan and documentation before implementation:
  tier, owner, non-merge rationale, and cleanup/archive rule. Generated local
  evidence must remain visible unless explicitly deleted or covered by a
  documented cleanup plan; do not add ignore rules merely to make the working
  tree appear clean.
- Revisit when: a V1 gate genuinely needs a separate import, validation, or
  repair evidence boundary that cannot be represented in an existing artifact

### DEC-061: User revision loop is plan-and-compare before applied edits

- Recorded: `2026-07-04`
- Status: `active`
- Decision: `V1-06` treats user revision notes as explicit planning evidence.
  The `revise` gate may classify the note, bind the current timeline and cut
  review, generate manual revision actions, and compare current versus revised
  candidates, but it must not mutate the canonical timeline or claim the
  revision has been applied.
- Rationale: a mature editing skill must respond to notes like shorter,
  stronger hook, keep/remove segment, change ending, reduce subtitles, or reduce
  BGM. Applying those changes safely is a separate gate because it can affect
  timing, rhythm, BGM fit, preview/final validation, and downstream NLE handoff.
- Consequence: V1-06 writes one canonical `revision_plan.json` plus
  `revision_plan.md`; status, version comparison, warnings, and forbidden flags
  stay inside those artifacts. Later applied-edit work must open a separate
  gate and prove timeline mutation, preview regeneration, BGM/rhythm freshness,
  and acceptance recovery explicitly.
- Revisit when: the project opens controlled revision execution or a release
  packaging gate that consumes revision plans.

### DEC-062: V2 starts with real-video aesthetic baselines

- Recorded: `2026-07-04`
- Status: `active`
- Decision: After `v0.29.0`, the next capability program is `V2-01 Real Video
  Aesthetic Baseline`, not more release packaging or isolated engineering
  artifacts. V2 progress must be measured by real-video edit quality: duration
  recommendation, highlight and weak-area scoring, multiple edit concepts, BGM
  and text/rhythm fit, first-cut self-review, and second-cut candidate planning.
- Rationale: V0 and V1 proved the local pipeline, final export, BGM handling,
  revision planning, and controlled timeline promotion. The remaining gap is
  aesthetic judgment. More schema, tests, FCPXML fields, or status surfaces will
  not make the system cut better unless they serve a real-video creative
  baseline.
- Consequence: `docs/CURRENT_BATCH.md` owns the active `V2-01` task list with
  twelve countable outcomes. Supporting fields, schemas, tests, refactors, and
  bug fixes remain non-counting work. The primary benchmark is the real
  `runs/chenhaoyu_klein_blue` project and the `克莱因蓝的独白` source video;
  additional benchmarks are added later under `V2-09`.
- Revisit when: `V2-01` produces a real-video baseline report and either
  justifies `V2-02` evidence fusion or exposes a stronger blocker.
