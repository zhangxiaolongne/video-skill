...
- committed proposal execution readiness plan schema
- `propose` writing deterministic `proposal_execution_readiness_plan.json`
- execution readiness plan records five blocked sub-stages: secret-source
  selection, credential access, execution planning, provider call preflight,
  and output capture planning, with no selected secrets, credential reads,
  model calls, network access, execution allowance, execution, raw output
  capture, or proposal content
...
- status and doctor visibility for malformed proposal execution readiness plans
