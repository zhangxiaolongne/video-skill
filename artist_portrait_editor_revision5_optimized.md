...
> **当前开发闸门**：V2-01 Real Video Aesthetic Baseline。当前批次只规划真实视频审美基线，不把字段、schema、测试、重构或局部 bugfix 当成版本进度。具体执行任务、验收和发布状态分别以 `docs/CURRENT_BATCH.md`、`docs/DEVELOPMENT_PROGRESS.md`、`docs/RELEASES.md` 与 `docs/current_progress.json` 为准。
...
当前 active gate 是 `V2-01 Real Video Aesthetic Baseline`。母版只记录战略边界：

- V1-01 到 V1-08 已发布为 `v0.29.0`，证明工程流水线、时长/评分/时间线、声音/BGM、复审、修订、修订提升和 release packaging 可用。
...
V2-01 的十三项可数任务、验收标准和 JSON 治理要求只在 `docs/CURRENT_BATCH.md` 维护。V2-V4 长线阶段表在第 18 节维护。
...
8. `V1-08` 修订提升与 V1 release packaging：已发布为 `v0.29.0`；把明确修订应用提升为 canonical timeline，并让 preview/final 重新绑定 revised timeline。
