...
> **当前开发闸门**：V3-05 NLE Round-Trip Plus，本地已完成、等待作为一个完整版本发布。V3-04 已发布；V3-06 尚未启动。具体执行任务、验收和发布状态分别以 `docs/CURRENT_BATCH.md`、`docs/DEVELOPMENT_PROGRESS.md`、`docs/RELEASES.md` 与 `docs/current_progress.json` 为准。
...
当前 active gate 是 `V3-05 NLE Round-Trip Plus`；`V3-04 A/B Version Review` 已发布。母版只记录战略边界：
...
- 第二版候选必须以用户明确选择的结构方案为入口，生成不覆盖一剪的独立真实媒体；源音频、BGM、文字、构图和语义证据必须分别记录实际应用状态，不能把技术可播放候选冒充成熟成片。
- 真实视频基准必须覆盖舞台人物、访谈/口播和活动/宣传混剪三类；允许某类暂时只有真实输入基线，但必须保留缺失闭环，不能用同一素材换标签或合成 fixture 填数。
- V2 release 证明的是“可讨论的一剪和有方向的二剪候选”与三类真实基准，不是成熟剪辑师完成态；`degraded`、`not_publishable` 和输入基线必须继续作为正式发布事实保留。
- 多版本策略必须在同一目标时长下形成情绪、燃向、叙事、人物高光四套物质上不同的选段/顺序和视听政策；策略名不能代替语义证据，缺少转录时情绪与叙事置信度必须降级。
- 风格系统必须把内容/成片形态、审美风格、创意策略、创意技法和情绪曲线分层组合，不能把素材类型冒充审美风格，也不能把示例列表写成封闭上限。规则支持 `follow / bend / break`；任何主动突破必须解释形式、感受、意义、风险、播放验证和失败回退。模板或组合必须由用户显式选择后才能应用。
- 交互修订必须保留一句反馈中的全部可识别意图，把强度、范围、优先级、证据、跨域连带影响和播放验收写清；“高级、快、少字、别压人声、结尾有力”不能退化成单个关键词，也不能在没有应用证据时声称已经完成。
- 多版本复审必须区分真实渲染、时间线候选和仅计划版本，逐域展示证据、置信度与缺失项。只有至少两个版本在同一目标上都有足够可靠证据时才能说某版领先；不得给出脱离目标的总冠军或自动替用户选版。
- NLE 交付必须把源文件身份、路径、哈希、版本身份、镜头区间、marker、cue 和音频指导一起交给剪辑师。写出 FCPXML/EDL/CSV 只证明候选文件存在；只有外部导入、relink、时间线、音频、播放和回导证据全部完成后，才能声称 round-trip 成功。
...
2. `V2-02` Frame Composition And Reframing：把真实画面布局变成可执行的编辑约束。系统必须
   ...
   允许的裁切/重构图候选。应用必须经过显式逐段选择并绑定当前时间线、最终成片、构图审查
   和抽样帧指纹；拒绝候选与受保护区域损失必须阻断，人物框风险和相邻段裁切中心跳变必须
   可见。系统可渲染独立播放候选验证时长、画布和音频保持，但不得覆盖 canonical timeline/
   final，不得把抽样帧安全伪装成完整运动安全或成熟审美验收。
