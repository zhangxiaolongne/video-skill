...
> **当前开发闸门**：V0-043 phrase-level manual edit guidance gate。可从 current rhythm plan、current timeline、optional BGM rhythm intelligence 与 optional rhythm media QC 生成 deterministic phrase-level manual edit guidance，覆盖 subtitle entrance、transition review、pause/breathing room、ducking review、BGM phrase reference、cut-to-cue review、ending style、source risk、QC repair 与 editor handoff guidance。不得自动选曲、移动剪辑点、mutate timeline、自动 fit music、渲染媒体、联网、从 CLI 调模型、从 PCM energy windows 伪造 BPM/beat grid、把 guidance 当作 execution plan、接入付费 API、API key、远程 provider、隐藏模型调用或 image generation/editing。
