...
> **当前开发闸门**：V0-010c 文本模型闸门契约。阶段 A、V0-003、V0-004、V0-005、V0-006、V0-007、V0-008、V0-009、V0-010a 与 V0-010b 已作为工程、媒体扫描、固定窗口切分、PySceneDetect 场景切分、本地转写、关键帧缓存、基础证据分析、分析驱动素材地图、提案就绪与提案上下文验收；当前只允许在既有确定性本地链路之后增加 `TextModelGate` 数据契约、`.artist-portrait/data/text_model_gate.json`、text-model gate 状态/doctor 诊断，以及 `propose` 对 policy/capability/生成器未开放原因的可审计阻塞。不得实现 OpenCV/视觉模型分类、BGM 选择、完整创作提案生成、时间线生成或预览渲染；不得执行模型调用或生成 fake/template/model-free proposals 冒充 creative_mode 成功。
...
- 当前 V0-010c text model gate contract 仍保持本地、无远程模型调用、无联网、无 image generation / editing 调用；`propose` 只从本地 ledger 生成 `proposal_context.json`，再从 project policy 和 capability 生成 `text_model_gate.json`，随后必须阻塞并记录原因，不得调用模型或输出假提案。
...
阶段 A 已完成基础工程验收，V0-003 已完成媒体扫描基础，V0-004 已完成固定窗口切分基础，V0-005 已完成 PySceneDetect 场景切分闸门，V0-006 已完成本地转写闸门，V0-007 已完成关键帧缓存闸门，V0-008 已完成基础证据分析闸门，V0-009 已完成分析驱动素材地图闸门，V0-010a 已完成提案就绪闸门，V0-010b 已完成提案上下文闸门。当前允许实现 V0-010c 文本模型闸门契约：
