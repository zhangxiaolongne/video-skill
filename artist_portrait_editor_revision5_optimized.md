...
> **当前开发闸门**：V0-051 FCPXML repair execution evidence import gate。可 byte-exact quarantine 外部 FCPXML repair execution record candidate，并根据当前 repair dry-run、approval record、repair plan、draft 与 import-review evidence 审查 action evidence，分类 accepted / rejected / missing / skipped。不得执行 repair commands、由 CLI 导入 Final Cut Pro 或任何 NLE、relink source media、渲染媒体、mutate canonical timeline、移动剪辑点、执行 editor instructions、自动选曲或自动 fit、联网、从 CLI 调模型、使用 image2/image generation/editing，或把 execution evidence 当作 repair success / acceptance success / edits applied 的证明。
...
阶段 A 至 V0-051 已完成 proposal generation、canonical timeline、multi-source BGM fitting、low-resolution preview rendering、preview QC、controlled local final export、local BGM technical intelligence、BGM recommendation review、beat-engine adapter evidence gate、recommendation-to-fit selection gate、recommendation-fit review gate、BGM fit controls gate、project acceptance gate、acceptance profile gate、real-media acceptance fixture gate、acceptance repair-plan gate、acceptance repair approval gate、repair execution dry-run gate、repair execution handoff gate、BGM/edit rhythm planning gate、rhythm media QC gate、rhythm acceptance integration gate、rhythm manual repair planning gate、guided workflow planning gate、workflow execution evidence review、workflow evidence repair planning gate、workflow repair approval dry-run gate、workflow repair execution review、accumulated workflow/rhythm release hardening gate、workflow repair evidence refresh guidance gate、BGM rhythm intelligence gate、phrase-level manual edit guidance gate、operator runbook usability gate、editor package handoff gate、NLE interchange planning gate、supervised FCPXML draft writer gate、FCPXML import-review evidence gate、FCPXML import/relink repair planning gate、FCPXML repair approval/dry-run packaging gate 与 FCPXML repair execution evidence import gate。当前开放 V0-051 FCPXML repair execution evidence import gate：
...
→ cue_sheet.csv
→ editor_handoff.json
→ nle_interchange_plan.json
→ nle_interchange_plan.md
→ nle_interchange_map.csv
→ nle_interchange_handoff.json
→ fcpxml_draft.json
→ fcpxml_validation.json
→ draft.fcpxml
→ fcpxml_review.md
→ fcpxml_handoff.json
→ fcpxml_import_review_candidate_quarantine.json
→ fcpxml_import_review.json
→ fcpxml_import_review.md
→ fcpxml_import_review_handoff.json
→ fcpxml_repair_plan.json
→ fcpxml_repair_plan.md
→ fcpxml_repair_handoff.json
→ fcpxml_repair_approval_request.json
→ fcpxml_repair_approval_request.md
→ fcpxml_repair_approval_handoff.json
→ fcpxml_repair_approval_record_quarantine.json
→ fcpxml_repair_approval_record.json
→ fcpxml_repair_approval_record.md
→ fcpxml_repair_dry_run.json
→ fcpxml_repair_dry_run.md
→ fcpxml_repair_dry_run_handoff.json
→ fcpxml_repair_execution_record_quarantine.json
→ fcpxml_repair_execution_review.json
→ fcpxml_repair_execution_review.md
→ fcpxml_repair_execution_handoff.json
...
FCPXML repair execution evidence 自动执行、自动 relink、自动 import、自动 render、自动 timeline mutation 或自动成功晋级
...
phrase-level manual edit guidance 自动选曲、自动 fit、移动剪辑点、mutate timeline、渲染媒体、调用模型、联网、使用 image2 或把 guidance 当作 execution plan
operator runbook 自动执行 workflow/repair commands、自动补跑 pipeline、渲染媒体、mutate timeline、移动剪辑点、自动选曲、自动 fit、调用模型、联网、使用 image2 或把 mixed extracted video audio 当作 clean BGM
editor package 自动渲染媒体、mutate timeline、移动剪辑点、执行 editor instructions、自动选曲、自动 fit、调用模型、联网、使用 image2 或把 package 当作 edits applied 的证明
NLE interchange plan 写 FCPXML/EDL/Resolve project file、渲染媒体、mutate timeline、移动剪辑点、执行 editor instructions、自动选曲、自动 fit、调用模型、联网、使用 image2 或把 mapping candidates 当作 edits applied 的证明
FCPXML draft 导入 Final Cut Pro 或任何 NLE、渲染媒体、mutate canonical timeline、移动剪辑点、执行 editor instructions、自动选曲、自动 fit、调用模型、联网、使用 image2、声称 source relink 成功或把 draft 当作 edits applied 的证明
FCPXML import-review 由 CLI 导入 Final Cut Pro 或任何 NLE、渲染媒体、mutate canonical timeline、移动剪辑点、执行 editor instructions、自动选曲、自动 fit、调用模型、联网、使用 image2、把 external import evidence 当作 project acceptance success 或 edits applied 的证明
FCPXML repair planning 由 CLI 导入 Final Cut Pro 或任何 NLE、relink source media、渲染媒体、mutate canonical timeline、移动剪辑点、执行 editor instructions、自动选曲、自动 fit、调用模型、联网、使用 image2、声称 repair success 或 edits applied
FCPXML repair approval/dry-run 执行 repair commands、由 CLI 导入 Final Cut Pro 或任何 NLE、relink source media、渲染媒体、mutate canonical timeline、移动剪辑点、执行 editor instructions、自动选曲、自动 fit、调用模型、联网、使用 image2、把 approval/dry-run 当作 repair success 或 edits applied
