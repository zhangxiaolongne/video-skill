...
judgment, optional local model/tool/search/image-generation handoffs, and
explicit user-provided public materials. It may recommend duration options,
score real highlight/weak-area candidates, compare multiple edit concepts,
review final-frame composition and safe reframe/crop candidates, review
BGM/source-audio/text/rhythm risks, review first-cut weaknesses, and prepare a
second-cut candidate plan. It must keep provenance visible, avoid paid APIs and
hidden network/provider calls, not fabricate unseen content understanding, not
...
an explicit user target duration or recommend short/standard/extended duration
options with evidence, platform assumptions, risks, BGM/source-audio guidance,
workflow ordering, state/run audit, and schema/test coverage. It must not render
media, mutate timelines, move edit points, select or fit music automatically,
call models from the CLI, access the network, use image generation/editing,
fabricate content understanding, or treat mixed extracted video audio as clean
BGM.

V1-02 may generate deterministic clip-score artifacts from local clip,
transcript, keyframe, analysis, and edit-brief evidence. It may write canonical
`.artist-portrait/data/clip_scores.jsonl` and `output/clip_score_report.md`,
score speech overlap, transcript density, local audio RMS, scene/boundary
evidence, keyframe coverage/cluster IDs, analysis confidence, duration fit, and
source risk penalties, then feed clip scores into proposal context. It must not
fabricate visual semantics, render media, mutate timelines, move edit points,
select or fit music automatically, call models from the CLI, access the
network, or use image generation/editing.

V1-03 may generate deterministic aesthetic timeline drafts from the selected
canonical proposal, current edit brief, current clip scores, local clips, and
source ledgers. It may write the canonical `output/timeline_draft.json` and
timeline review artifacts with hook/build/payoff structure, multiple duration
variant planning, keep/drop reasons, score bindings, continuity checks, and
explicit no-fabricated-content flags. It must not render media, select or fit
music automatically, fabricate visual semantics, call models from the CLI,
access the network, use image generation/editing, or hide generated evidence.

V1-04 may generate deterministic sound/BGM decision artifacts from the current
canonical timeline, edit brief, optional BGM candidate ledger, optional BGM fit
plan, optional BGM analysis, and optional BGM rhythm intelligence. It may write
canonical `.artist-portrait/data/sound_decision.json` and
`output/sound_decision.md`, choosing a documented strategy among original
audio, direct BGM, video-extracted mixed audio, source embedded audio, silence,
ducking, fades, explicit fit policy, and beat-unavailable fallback. It must not
select music automatically, fit music automatically, render media, move edit
points, fabricate BPM or beat grids, call models from the CLI, access the
network, use image generation/editing, hide generated evidence, or treat
video-extracted mixed audio as clean BGM.

V1-05 may generate deterministic aesthetic cut-review artifacts from the
current canonical timeline, sound decision, optional rhythm plan, optional
preview/final-export validation, optional rhythm media QC, and optional edit
guidance. It may write canonical `.artist-portrait/data/cut_review.json` and
`output/cut_review.md`, review weak opening, dead space, ending strength,
rhythm mismatch, audio conflict, media-QC gaps, and manual second-pass actions.
It must not render media, mutate canonical timelines, move edit points, select
or fit music automatically, call models from the CLI, access the network, use
image generation/editing, or treat the second-pass plan as applied edits.

V1-06 may generate deterministic user revision-loop artifacts from the current
canonical timeline, current cut review, optional sound decision, optional rhythm
plan, optional preview/final-export validation, and an explicit user revision
note. It may write canonical `.artist-portrait/data/revision_plan.json` and
`output/revision_plan.md`, classify requests such as shorter, longer, stronger
hook, more emotional, keep/remove segment, change ending, reduce subtitles, and
reduce BGM, then compare the current version with manual revision candidates.
It must not render media, mutate canonical timelines, move edit points
automatically, select or fit music automatically, call models from the CLI,
access the network, use image generation/editing, or claim revision actions
were applied.

V1-07 may generate deterministic controlled revision-application artifacts from
the current canonical timeline, current revision plan, and an explicit
non-current revision candidate selection. It may write canonical
`.artist-portrait/data/revision_application.json` and
`output/revision_application.md`, build a revised timeline candidate inside the
canonical artifact, classify applied/preserved/manual-only/skipped/conflict
revision actions, recompute synchronized timeline/source segment timing for
safe candidate edits, and surface downstream preview/final/rhythm freshness
guidance. It must not mutate `output/timeline_draft.json`, promote a candidate
to the canonical timeline, render preview/final media, select or fit music,
move canonical edit points, call models from the CLI, access the network, use
image generation/editing, or claim revised media has been regenerated.

V1-08 may explicitly promote a current validated revision application into the
canonical `output/timeline_draft.json` only when the user supplies the exact
`revision_application_id`. It may write canonical
`.artist-portrait/data/revision_promotion.json` and
`output/revision_promotion.md`, atomically rewrite the canonical timeline with
the revised candidate segments, record baseline/promoted fingerprints, mark
downstream preview/final/rhythm/review/acceptance/handoff artifacts stale, and
prepare V1 release packaging after full validation. It must not render media
inside `promote-revision`, automatically select or fit music, fabricate BPM or
beat grids, call models from the CLI, access the network, use image
generation/editing, hide generated evidence, include local run media in release
commits, or claim downstream preview/final media has been regenerated until
those render commands actually run.

V1 JSON convergence rule: new capability gates default to one canonical machine
JSON plus one human Markdown report. Add separate JSON only for external
candidate quarantine/import, generated schema boundaries, media validation, run
audit, or release-critical repair/evidence chains. Ordinary handoff, review,
status, or summary data must be folded into the canonical artifact or Markdown.
Generated evidence must stay visible unless the user explicitly approves
deletion or a documented cleanup plan owns it; do not add ignore rules merely to
make the working tree appear clean.
...
V0-027 through V0-030 are superseded. Acceptance reports expose actionable next commands directly; the removed repair-plan, approval, dry-run, bundle, and external execution chains were redundant and are not implemented.
...
acceptance profiles, direct next-command reporting, guided workflow plans, reports, and generated real-media acceptance checks. It may
...
V0-050 and V0-051 are superseded. FCPXML retains explicit import-review evidence and a manual repair plan, but no simulated approval, dry-run, or execution-record chain.
...
- `docs/RELEASES.md`: current release state, current validation, and Git state
- `docs/archive/RELEASES_HISTORY.md`: detailed historical release outcomes

`docs/current_progress.json` is the machine-readable mirror. Historical outcomes
are consolidated in `docs/archive/RELEASES_HISTORY.md`; do not recreate per-version readiness,
...
- byte-exact candidate quarantine, semantic validation, and atomic canonical promotion
...
- direct acceptance-report next commands without simulated repair evidence chains
- `rhythm`
...
- deterministic `output/workflow_plan.md` and `output/workflow_agent_handoff.json`
- `workflow --execution-record <candidate.json>` with byte-exact quarantine and
  canonical `.artist-portrait/data/workflow_execution_review.json`
- deterministic `output/workflow_execution_review.md` and
  `output/workflow_execution_handoff.json`
- no workflow repair approval, dry-run, execution, or refresh artifact chain
