...
def run_quality_pass(workspace: Path) -> dict:
    ...
    cli("brief", "--project", str(project), "--quiet", expect=(0, 1))
    scores = cli("score", "--project", str(project), "--json", expect=(0, 1))
    if scores["output"] != ".artist-portrait/data/clip_scores.jsonl":
        raise SystemExit("BGM/rhythm quality score did not write clip score ledger")
    ...
    no_file_sound = cli("sound", "--project", str(project), "--json", expect=(0, 1))[
        "sound_decision"
    ]
    no_file_modes = {item["mode"]: item for item in no_file_sound["input_modes"]}
    if no_file_modes["no_file_yet"]["status"] != "missing":
        raise SystemExit("no-file-yet sound decision did not expose missing BGM input")
    if no_file_sound["automatic_music_selection"] is not False:
        raise SystemExit("no-file-yet sound decision selected music automatically")
    ...
    if sound["selected_strategy"] not in {"bgm_ready_for_mix", "original_audio_with_bgm"}:
        raise SystemExit("sound decision did not bind the explicit direct BGM fit")
    if sound["mixed_audio_warning_count"] < 1:
        raise SystemExit("sound decision missed video-extracted mixed-audio risk")
    ...
    if cut_review["automatic_music_selection"] is not False:
        raise SystemExit("cut review selected music")
    ...
    if cut_review["edit_points_moved"] is not False:
        raise SystemExit("cut review moved edit points")
    delivery = cli(
        "acceptance",
        "--project",
        str(project),
