...
description: Artist portrait video workflow. Use when Codex needs deterministic media ledgers, evidence analysis, an analysis-led material map, evidence-grounded creative proposals generated by the active Codex/ChatGPT host Agent then quarantined, validated, reviewed, and atomically promoted, a canonical timeline after explicit proposal selection, multi-source BGM ingestion and BGM fitting with explicit controls, validated local beat-engine evidence when available, explicit recommendation-to-fit BGM selection, recommendation-fit review, local low-resolution preview rendering with QC, controlled local final export, or project acceptance reporting, all without paid APIs, API keys, or network calls. Covers local validation, review/doctor, and boundaries before automatic music selection, beat-synced editing, vision, or image tools.
---
...
forbidden capability flags. It does not generate proposals, choose music, fit
music, render media, call models, access the network, or repair missing
artifacts.
